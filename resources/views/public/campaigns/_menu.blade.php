	...
	<div class="col-md-12">
		<ul class="nav nav-tabs nav-tabs-top nav-underline">
			...
			<li class="nav-item {{ $menu == 'template' ? 'active' : '' }}">
				<a href="javascript:;" onclick="popupwindow('{{ action('Pub\CampaignController@preview', $campaign->uid) }}', `{{ $campaign->name }}`, 800)" class="nav-link">
...
<script>
	...
	function goToDownLoad(logtype) {
		downloadWindow = window.open('{{ action('Pub\CampaignController@trackingLogDownload', ['uid' => $campaign->uid]) }}?logtype=' + logtype, '_blank');
	}
