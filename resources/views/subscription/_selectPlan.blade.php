<div class="card subscription-step {{ ($invoice->type !== \Acelle\Model\InvoiceNewSubscription::TYPE_NEW_SUBSCRIPTION) ? 'disabled' : '' }}">
    <a 
            ...
            <div>
                <h5 class="fw-600 mb-0 fs-6 text-start">
                    {{ trans('messages.subscription.plan_chosen', [
                        'plan' => $invoice->mapType()->getBillingInfo()['plan']->name
                    ]) }}
                </h5>
                <p class="m-0 text-muted">{!! $invoice->mapType()->getBillingInfo()['description'] !!}</p>
            </div>
            <div class="ms-auto">
